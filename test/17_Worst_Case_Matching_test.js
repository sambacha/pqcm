...
contract('DappToken', function (accounts) {
  it('should approve transfer of 100 tokens from account[0] to the CallMarket contract ', async () => {
    ...
    const receipt = await DappTokenInstance.approve(
      CallMarketaddress,
      2000000,
      { from: accounts[0] },
    );
...
describe('CallMarket', function (accounts) {
  ...
  it('should show the Ether balances of traders before the trade', async () => {
    ...
    console.log(
      'The Ether balance of account[1] (Alice) before the trade is:',
      AliceEtherBalance,
    );
    console.log(
      'The Ether balance of account[0] (Bob) before the trade  is:',
      BobEtherBalance,
    );
    console.log('********************************************');
  });
  ...
  it('should deposit tokens from accounst[0] to the CallMarket contract', async () => {
    ...
    const receipt = await CallMarketInstance.depositToken(
      tokenaddress,
      2000000,
      { from: accounts[0] },
    );
  ...
  it('should submit asks from accounst[0]', async () => {
    ...
    for (let j = 70; j >= 1; j--) {
      ...
      await CallMarketInstance.submitAsk(j, randomnumber, {
        from: accounts[0],
      });
      array.push(j);
  ...
  it('should submit Bids from accounst[1]', async () => {
    ...
    for (let j = 71; j <= 140; j++) {
      var randomnumber =
        Math.floor(Math.random() * (maximum - minimum + 1)) + minimum;
      await CallMarketInstance.submitBid(j, randomnumber, {
        from: accounts[1],
      });
      array.push(j);
